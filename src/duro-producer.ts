...
export async function publish(publishOptions: PublishOptions): Promise<PubAck> {
  ...
  if (!subjectExists) throw new Error(`Subject ${subject} does not exist in stream ${streamName}`);
  ...
  try {
    ...
    console.log(`Message published to stream ${streamName} subject ${subject}, sequence ${ack.seq}`);
    return ack;
  } catch (err: any) {
